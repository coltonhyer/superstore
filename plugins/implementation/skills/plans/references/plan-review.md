...
Flag a plan that leaves required spec behavior undelivered, or assigns it to
code outside the repository without evidence, even when the plan states that
exclusion openly; plan approval cannot stand in for a scope decision. Behavior
the plan assigns to a named separate plan of the same specification is a scope
boundary to check, not a gap. Flag a task that merely defers required spec
behavior or an unresolved material scope, ownership, or authority boundary to
discovery. Discovery is sufficient only when its observable result and
completion observation are defined. When bounded inspection and ordinary
engineering judgment consistent with the approved spec can resolve a missing
owner, location, or verification approach, the plan may choose a new
within-scope file or check. Otherwise, identify the approval-blocking user
decision; do not treat the placeholder as a resolution.
